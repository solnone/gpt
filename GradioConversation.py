...
def create_conversation(openai_api_key, system_message, temperature, k):
...
def respond(message, chat_history, conversation, openai_api_key, system_message, temperature, k):
    if (len(message) > 0):
        if (conversation is None):
            conversation = create_conversation(openai_api_key, system_message, temperature, int(k))
...
with gr.Blocks() as demo:
    ...
    msg.submit(respond, [msg, chatbot, conversation, openai_api_key, system_message, temperature, k], [msg, chatbot, conversation])
